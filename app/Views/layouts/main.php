...
<body class="bg-gray-50 flex">
    ...
    <aside class="w-64 h-screen bg-[#0f172a] text-slate-300 border-r border-slate-800 fixed flex flex-col no-scrollbar">
        ...
        <nav class="mt-6 px-4 flex-1">
            ...
            <a href="/dashboard" class="flex items-center gap-3 px-4 py-3 rounded-xl mb-2 transition-all group <?= $current_uri == '/dashboard' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'hover:bg-slate-800 hover:text-white' ?>">
        ...
        <div class="p-4 border-t border-slate-800 mb-4">
            <div class="flex items-center gap-3 px-2 py-3 rounded-xl bg-slate-800/50">
                <div class="w-10 h-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-sm uppercase">
                    <?= isset($_SESSION['user_name']) ? substr($_SESSION['user_name'], 0, 2) : 'AD' ?>
                </div>
                ...
                <a href="/logout" class="ml-auto p-1 hover:text-red-400 transition-colors">
